/**
* @fileoverview Wrapper for ImageMagick. Tested with ImageMagick 6.6.7-1 2011-01-29 Q16 &mdash; <a href="http://www.imagemagick.org">http://www.imagemagick.org</a>
*/
...
var _pp_resize = function(src, dest, params) {
   ...
   return subprocess.command(cmd.join(" ")).replace(/[\n\r\t]/g,"");
};
...
var getIPTC = function(src) {   
   var info = (subprocess.command("identify -format " + _iptcFormatString + " " + src, {encoding: java.lang.System.getProperty("file.encoding")})).split("\n");
   var iptcData = {};
...
var getEXIF = function(src) {   
   ...
   info.forEach(function(item, index) {
      if (item) {
        ...
        exifData[keyVal[0]] = (keyVal[1] || "").replace(/[\n\r\t]/g,"");
      }
   });
...
var getSize = function(src) {
   return (subprocess.command("identify -format %G " + src)).split("x").map(function(item) {
...
var getColorspace = function(src) {
   return subprocess.command("identify -format %[colorspace] " + src).replace(/[\n\r\t]/g,"");
};
...
var getCompression = function(src) {
   return subprocess.command("identify -format %C " + src).replace(/[\n\r\t]/g,"");
};
...
var getQuality = function(src) {
   return parseInt(subprocess.command("identify -format %Q " + src).replace(/[\n\r\t]/g,""), 10);
};
 ...
 *        <ul>
 *        <li>brightness: brightness percentage of the watermark image. Default 50.
 *        <li>saturation: saturation percentage of the watermark image. 0 will add b/w watermark, 100 keeps the src image's colors. Default 50.
 *        <li>gravity: possible values: NorthWest, North, NorthEast, West, Center, East, SouthWest, South, SouthEast.
 *        <li>geometry: see http://www.imagemagick.org/script/command-line-processing.php#geometry
 *        </ul>
 */
var addWatermarkImage = function(src, watermark, dest, options) {
   var params = objects.merge(options, {
      brightness: 50,
      saturation: 50,
      gravity: "Center",
      geometry: "100%"
   });
   return subprocess.command("composite -watermark "
      + [params.brightness, params.saturation].join("x")
      + " -gravity " + params.gravity
      + " -geometry " + params.geometry
      + " " + [watermark, src, dest].join(" "));
};